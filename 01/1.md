...
- The Code contains eight Principles related to the behavior of an decisions made by professional software engineers, including practitioners, educators, managers, supervisors and policy makers, as well as trainees and students of the profession.
...
6. PROFESSION - Software engineers shall advance the integrity and reputation of the professional consistent with the public interest.
...
- You employer acts in an unethical way and releases a safety-critical system without finishing the testing of the system.
...
![](./img/insulin_pump_hardware.png)

## Activity model of the insulin pump

![](./img/insulin_pump_activity_model.png)
...
- Most mental health patients do not required dedicated hospital treatment but need to attend specialist clinics regularly where they can meet a doctor who has detailed knowledge of their problems.
...
![](./img/mentcare_system_organization.png)
...
- Weather stations collect data from a set of instruments that measure temperature and pressure, sunshine, rainfall, wind speed and wind direction.
    - The weather station includes a number of instruments that measure weather parameters such as the wind speed and direction, the ground and air temperatures, the barometric pressure and the rainfall over 24-hour period. Each of these instruments is controlled by a software system that takes parameter reading periodically and manages the data collected from the instruments.

## The weather station's environment

![](./img/weather_station_environment.png)
...
- The tools included in each version of the environment are chosen by teachers and learners to suit their specific needs.
    - These can be general applications such as spreadsheets, learning management application such as a Virtual Learning Environment (VLE) to manage homework submission and assessment, games and simulations.
...
- _Utility services_ that provide basic application-independent functionality and which may be used by other services in the system.
- _Application services_ that provide specific applications such as email, conferencing, photo sharing etc. and access to specific educational content such as scientific films or historical resources.
- _Configuration services_ that are used to adapt the environment with a specific set of application services and do define how services are shared between students, teachers and their parents.

## iLearn architecture

![](./img/ilearn_architecture.png)

## iLearn service integration

- _Integrated services_ are services which offer an API (application programming interface) and which can be accessed by other services through that API.Direct service-to-service communication is therefore possible.

- _Independent services_ are services which are simply accessed through a browser interface and which operate independently of other services. Information can only be shared with other services. Information can only be shared with other services through explicit user actions such as copy and paste; re-authentication may be required for each independent service.
...
- Software engineers have responsibilities to the engineering profession and society. They should not simply be concerned with technical issues.
- Professional societies publish code of conduct which set out the standards of behavior expected of their members.